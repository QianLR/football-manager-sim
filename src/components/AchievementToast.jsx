...
export default function AchievementToast({ toast }) {
  ...
  const title = toast.title || def.title;
  const hint = toast.hint || def.hint;
  ...
  return (
    <div className="fixed top-12 right-2 z-50">
      <div className="bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-2 max-w-xs">
        ...
        <div className="font-bold text-xs font-mono text-black leading-snug mt-0.5">{title}</div>
        {hint ? (
          <div className="text-[11px] text-gray-600 font-mono leading-snug mt-0.5">{hint}</div>
        ) : null}
      </div>
    </div>
  );
